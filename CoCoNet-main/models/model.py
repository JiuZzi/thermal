...
class Unet_resize_conv(nn.Module):
    def __init__(self, ablation='full'):
        super(Unet_resize_conv, self).__init__()

        valid = {'full', 'no_vgg', 'no_ca', 'no_ca_vgg'}
        if ablation not in valid:
            raise ValueError(f'ablation must be one of {sorted(valid)}, got {ablation}')
        self.ablation = ablation
        self.use_vgg = ablation in {'full', 'no_ca'}
        self.use_ca = ablation in {'full', 'no_vgg'}

        # self.vgg = load_vgg16("./core/Losses/vgg", cfg)
        # self.vgg.to_relu_1_2[0] = nn.Conv2d(4, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.se_32 = CAM_Module(32) if self.use_ca else None
        self.se_64 = CAM_Module(64) if self.use_ca else None
        self.se_128 = CAM_Module(128) if self.use_ca else None
        self.se_256 = CAM_Module(256) if self.use_ca else None

        # The VGG backbone is absent from the ``no_vgg`` architecture rather
        # than merely being skipped during forward propagation.
        self.vgg19 = Vgg19_Unet(vgg19_weights='place_holder') if self.use_vgg else None
        ...
        self.deconv4 = nn.Conv2d(256 * (3 if self.use_vgg else 1), 256, 3, padding=p)
        ...
        self.att_deconv7 = nn.Conv2d(128 * (3 if self.use_vgg else 1), 128, 3, padding=p)
        ...
        self.att_deconv8 = nn.Conv2d(64 * (3 if self.use_vgg else 1), 64, 3, padding=p)
    ...
    def forward(self, ir, vis):
        ...
        if self.use_ca:
            unet_out, _ = self.se_256(x)
        else:
            unet_out = x
        if self.use_vgg:
            if self.use_ca:
                vgg_v_out, _ = self.se_256(vis_4)
                vgg_i_out, _ = self.se_256(ir_4)
            else:
                vgg_v_out, vgg_i_out = vis_4, ir_4
            att_4 = torch.cat([unet_out, vgg_v_out, vgg_i_out], 1)
            x = self.deconv4(att_4)
        else:
            x = self.deconv4(unet_out)
        conv4 = self.bn4_2(self.LReLU4_2(self.conv4_2(x)))

        conv6 = F.upsample(conv4, scale_factor=2, mode='bilinear')  # 256

        if self.use_ca:
            unet_out, _ = self.se_128(conv3)
        else:
            unet_out = conv3
        if self.use_vgg:
            if self.use_ca:
                vgg_v_out, _ = self.se_128(vis_3)
                vgg_i_out, _ = self.se_128(ir_3)
            else:
                vgg_v_out, vgg_i_out = vis_3, ir_3
            att_7 = torch.cat([unet_out, vgg_v_out, vgg_i_out], 1)
        else:
            att_7 = unet_out
        att_7 = self.att_deconv7(att_7)
        ...
        if self.use_ca:
            unet_out, _ = self.se_64(conv2)
        else:
            unet_out = conv2
        if self.use_vgg:
            if self.use_ca:
                vgg_v_out, _ = self.se_64(vis_2)
                vgg_i_out, _ = self.se_64(ir_2)
            else:
                vgg_v_out, vgg_i_out = vis_2, ir_2
        maps.append(conv2)
